...
namespace kspp {
  tds_generic_avro_source::tds_generic_avro_source(topology &t,
      ...
      : partition_source<void, kspp::GenericAvro>(nullptr, partition), _started(false), _exit(false),
        _thread(&tds_generic_avro_source::thread_f, this),
        _impl(partition, table, t.consumer_group(), host, user, password, database, id_column, ts_column), _schema_registry(registry),
...
//  void tds_generic_avro_source::parse(const PGresult* result) {
//    if (!result)
//      return;
//
//    // first time?
