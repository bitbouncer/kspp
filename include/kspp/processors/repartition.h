...
namespace kspp {
  ...
  public:
    repartition_by_foreign_key(std::shared_ptr<cluster_config> config,
        ...
        : event_consumer<K, V>(), partition_processor(source.get(), source->partition()), source_(source),
          routing_table_(routing_table), topic_sink_(topic_sink), repartition_codec_(repartition_codec) {
