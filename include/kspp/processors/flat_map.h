...
namespace kspp {
  ...
  public:
    ...
    flat_map(std::shared_ptr<cluster_config> config, std::shared_ptr<partition_source<SK, SV>> source, extractor f)
        : event_consumer<SK, SV>(), partition_source<RK, RV>(source.get(), source->partition()), source_(source),
          extractor_(f) {
...
template<class SK, class SV, class RK, class RV>
void insert(kspp::flat_map<SK, SV, RK, RV> *flatMap, const RK &k, const RV &v,
...
template<class SK, class SV, class RK, class RV>
void insert(kspp::flat_map<SK, SV, RK, RV> *flatMap, const RK &k, std::shared_ptr<const RV> p,
            int64_t ts = kspp::milliseconds_since_epoch()) {
...
template<class SK, class SV, class RK, class RV>
void insert(kspp::flat_map<SK, SV, RK, RV> *flatMap, const RK &k, std::shared_ptr<RV> p,
            int64_t ts = kspp::milliseconds_since_epoch()) {
  auto kr = std::make_shared<kspp::krecord<RK, RV >>(k, p, ts);
  flatMap->push_back(kr);
}
...
template<class SK, class SV, class RV>
void insert(kspp::flat_map<SK, SV, void, RV> *flatMap, std::shared_ptr<const RV> p,
            int64_t ts = kspp::milliseconds_since_epoch()) {
