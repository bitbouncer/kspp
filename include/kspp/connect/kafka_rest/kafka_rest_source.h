...
#include <kspp/connect/kafka_rest/kafka_rest_consumer.h>

#pragma once

namespace kspp {
  template<class K, class V, class KEY_CODEC, class VAL_CODEC>
  class kafka_rest_source_base : public partition_source<K, V> {
    static constexpr const char* PROCESSOR_NAME = "kafka_rest_source";
  public:
    virtual ~kafka_rest_source_base() {
      close();
    }
  ...
  protected:
    kafka_rest_source_base(std::shared_ptr<cluster_config> config,
                      std::string topic,
                      int32_t partition,
        ...
        , _started(false)
        , _exit(false)
        , _thread(&kafka_rest_source_base::thread_f, this)
  ...
  template<class K, class V,  class KEY_CODEC, class VAL_CODEC>
  class kafka_rest_source : public kafka_rest_source_base<K, V, KEY_CODEC, VAL_CODEC> {
  public:
    kafka_rest_source(topology &t,
                 int32_t partition,
                 std::string topic,
                 std::shared_ptr<KEY_CODEC> key_codec = std::make_shared<KEY_CODEC>(),
                 std::shared_ptr<VAL_CODEC> val_codec = std::make_shared<VAL_CODEC>())
        : kafka_rest_source_base<K, V, KEY_CODEC, VAL_CODEC>(
  ...
  class kafka_rest_source<void, V, void, VAL_CODEC> : public kafka_rest_source_base<void, V, void, VAL_CODEC> {
  public:
    kafka_rest_source(topology &t,
                 int32_t partition,
                 std::string topic,
                 std::shared_ptr<VAL_CODEC> val_codec = std::make_shared<VAL_CODEC>())
        : kafka_rest_source_base<void, V, void, VAL_CODEC>(
        t.get_cluster_config(),
        topic,
        partition,
        t.consumer_group(),
  ...
  class kafka_rest_source<K, void, KEY_CODEC, void> : public kafka_rest_source_base<K, void, KEY_CODEC, void> {
  public:
    kafka_rest_source(topology &t,
                 int32_t partition,
                 std::string topic,
                 std::shared_ptr<KEY_CODEC> key_codec = std::make_shared<KEY_CODEC>())
        : kafka_rest_source_base<K, void, KEY_CODEC, void>(
        t.get_cluster_config(),
        topic, partition,
        t.consumer_group(),
        std::chrono::system_clock::from_time_t(0),
