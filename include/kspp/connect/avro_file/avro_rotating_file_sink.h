...
namespace kspp {
  class avro_rotating_file_sink : public topic_sink<void, kspp::generic_avro> {
    static constexpr const char* PROCESSOR_NAME = "avro_rotating_file_sink";
  public:
    avro_rotating_file_sink(std::shared_ptr<cluster_config> config, std::string parent_path, std::string base_name, std::chrono::seconds window_size)
    ...
    ~avro_rotating_file_sink() override {
      this->flush();
      this->close();
    }
