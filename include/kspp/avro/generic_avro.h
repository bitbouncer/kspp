  ...
  public:
    ...
    public:
      ...
      T get(const std::string &member) const {
        ...
        throw std::invalid_argument(name() + "." + member + ":  wrong type, expected:" +
                                    avro_utils::to_string(avro_utils::cpp_to_avro_type<T>()) + ", actual: " +
      ...
      std::optional<std::string> get_optional_as_string(const std::string &member) const {
        ...
        switch (datum.type()) {
          ...
          default:
            throw std::invalid_argument(name() + "." + member + ": , cannot convert to string, actual type: " +
                                        avro_utils::to_string(datum.type()));
        }
      }
    ...
    generic_avro::generic_record record() const {
      if (generic_datum_->type() == avro::AVRO_RECORD) {
        ...
        throw std::invalid_argument(
            std::string("wrong type, expected: ") + avro_utils::to_string(avro::AVRO_RECORD) + " actual: " +
            avro_utils::to_string(generic_datum_->type()));
      }
    }
...
template<>
struct avro::codec_traits<kspp::generic_avro> {
...
kspp::avro_utils::avro_utils<kspp::generic_avro>::valid_schema(const kspp::generic_avro &dummy) {
