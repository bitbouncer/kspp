...
public:
  ...
  public:
    generic_record(avro::GenericRecord &record) : record_(record) {}

    inline const avro::GenericDatum &
    get_generic_datum(const std::string &member) const {
      if (!record_.hasField(member))
        throw std::invalid_argument(name() + "." + member +
                                    ": no such member, actual: " + to_json());
    ...
    template <class T> T get(const std::string &member) const {
      if (!record_.hasField(member))
      ...
      throw std::invalid_argument(
          name() + "." + member + ":  wrong type, expected:" +
          avro_utils::to_string(avro_utils::cpp_to_avro_type<T>()) +
          ", actual: " + avro_utils::to_string(datum.type()));
    }

    template <class T> void set(const std::string &member, const T &val) {
      if (!record_.hasField(member))
        throw std::invalid_argument("no such member: " + member);
    ...
    get_optional_as_string(const std::string &member) const {
      if (!record_.hasField(member))
      ...
      default:
        throw std::invalid_argument(
            name() + "." + member +
            ": , cannot convert to string, actual type: " +
            avro_utils::to_string(datum.type()));
      }
    }
    ...
    std::string name() const { return record_.schema()->name().fullname(); }
  ...
  inline int32_t schema_id() const { return schema_id_; }

  inline avro::Type type() const { return generic_datum_->type(); }

  generic_avro::generic_record record() const {
    if (generic_datum_->type() == avro::AVRO_RECORD) {
      return generic_avro::generic_record(
          generic_datum_->value<avro::GenericRecord>());
    } else {
      throw std::invalid_argument(
          std::string("wrong type, expected: ") +
          avro_utils::to_string(avro::AVRO_RECORD) +
          " actual: " + avro_utils::to_string(generic_datum_->type()));
    }
  }
...
template <>
inline std::string
kspp::avro_utils::avro_utils<kspp::generic_avro>::schema_name(
    const kspp::generic_avro &dummy) {
  return normalize(*dummy.valid_schema());
}

template <>
inline std::string
kspp::avro_utils::avro_utils<kspp::generic_avro>::schema_as_string() {
  kspp::generic_avro dummy;
  return normalize(*dummy.valid_schema());
}

template <>
inline std::shared_ptr<const avro::ValidSchema>
kspp::avro_utils::avro_utils<kspp::generic_avro>::valid_schema(
    const kspp::generic_avro &dummy) {
