...
namespace kspp {
  ...
  class s3_offset_storage2 : public offset_storage {
  public:
  s3_offset_storage2(std::string host, std::string s3_bucket, std::string key, std::string _access_key, std::string _secret_key, bool use_ssl);
  ~s3_offset_storage2() override;
  private:
  int64_t load_offset(int timeout_ms) override;
  void persist_offset(int64_t offset, int timeout_ms) override;
  const std::string _host;
  const std::string _s3_bucket;
  const std::string _s3_object_name;
  const std::string _access_key;
  const std::string _secret_key;
