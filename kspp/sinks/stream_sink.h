...
namespace kspp {
  ...
  void stream_sink_print(std::shared_ptr<kevent < K, V>>

  ev,
  std::ostream &_os, kspp::text_serdes
  * _codec) {
  _os << "ts: " << ev->
...
template<class V>
void stream_sink_print(std::shared_ptr<kevent < void, V>>
...
template<class K>
void stream_sink_print(std::shared_ptr<kevent < K, void>>
...
public:
  ...
  stream_sink(topology_base &topology, std::shared_ptr<partition_source < K, V>>

  source,
  std::ostream *os
  )
  :
  partition_sink<K, V>(source
  ->
