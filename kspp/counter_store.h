...
class kcounter_store
{
  public:
  enum { MAX_KEY_SIZE = 10000, MAX_VALUE_SIZE = 100000 };

  class iterator : public ktable_iterator_impl<K, V>
  {
    public:
  ...
  kcounter_store(std::string topic, int32_t partition, boost::filesystem::path storage_path, std::shared_ptr<codec> codec) 
    : _codec(codec)
    , _topic(topic)
