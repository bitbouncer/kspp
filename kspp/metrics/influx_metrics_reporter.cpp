...
influx_metrics_reporter::influx_metrics_reporter(kspp::topology_builder& builder, std::string topic, std::string prefix, std::string tags)
  : _run(true)
  , _topic(topic)
  , _prefix(prefix)
  , _tags(tags)
  ...
  _thread = std::make_shared<std::thread>([this]() {
    std::string base_string = _prefix + (_tags.size() ? "," + _tags : "");
    ...
    while (_run) {
      ...
      if (next_time_to_send <= kspp::milliseconds_since_epoch()) {
        ...
        for (auto i : _reported_p_topologys) {
          i->for_each_metrics([this, &base_string, &measurement_time_str, &measurement_time](kspp::metric& m) {
            ...
            std::string value = base_string + (measurement_tags.size() ? "," + measurement_tags : "") + " " + m.name() + "=" + std::to_string(m.value()) + "i " + measurement_time_str;
...
void influx_metrics_reporter::add_metrics(std::shared_ptr<topic_topology> p) {
  p->init_metrics();
  // maybe we need a mutex here... TBD
  _reported_t_topologys.push_back(p); 
}

void influx_metrics_reporter::add_metrics(std::shared_ptr<partition_topology> p) {
  p->init_metrics();
  // maybe we need a mutex here... TBD
  _reported_p_topologys.push_back(p);
}
