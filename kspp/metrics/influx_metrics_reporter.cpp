...
influx_metrics_reporter::influx_metrics_reporter(kspp::topology_builder& builder, std::string topic, std::vector<std::shared_ptr<topic_topology>> topologys, std::string prefix, std::string tags)
  : _run(true)
  , _topic(topic)
  , _reported_topologys(topologys)
  , _prefix(prefix)
  , _tags(tags)
  , _hostname(hostname()) {
  ...
  for (auto  i : _reported_topologys)
    i->init_metrics();
