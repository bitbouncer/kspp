...
std::shared_ptr<avro::RecordSchema> schema_for_table_row(std::string schema_name, std::shared_ptr<PGresult> res) {
  std::shared_ptr<avro::RecordSchema> record_schema = std::make_shared<avro::RecordSchema>(schema_name);
...
std::shared_ptr<avro::ValidSchema> valid_schema_for_table_row(std::string schema_name, std::shared_ptr<PGresult> res) {
  return std::make_shared<avro::ValidSchema>(*schema_for_table_row(schema_name, res));
}
...
valid_schema_for_table_key(std::string schema_name, const std::vector<std::string> &keys,
  ...
  return std::make_shared<avro::ValidSchema>(*schema_for_table_key(schema_name, keys, res));
}
...
to_avro(std::shared_ptr<avro::ValidSchema> schema, std::shared_ptr<PGresult> res) {
  ...
  std::vector<std::shared_ptr<avro::GenericDatum>> result;
  result.reserve(nRows);
  for (int i = 0; i < nRows; i++) {
    std::shared_ptr<avro::GenericDatum> gd = std::make_shared<avro::GenericDatum>(*schema);
...
static std::string escapeSQLstring(std::string src) {
