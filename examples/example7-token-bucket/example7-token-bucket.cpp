...
int main(int argc, char **argv) {
...

  {
    ...
    auto limited_streams = topology->create_partition_processors<kspp::rate_limiter<std::string, void>>(filtered_streams, 1s, 10);

    auto word_counts = topology->create_partition_processors<kspp::count_by_key<std::string, size_t, kspp::text_codec>>(limited_streams, 2000s, codec);
    
    auto thoughput_limited_streams = topology->create_partition_processors<kspp::thoughput_limiter<std::string, size_t>>(word_counts, 10); // messages per sec
    
    auto sinks = topology->create_partition_processors<kspp::stream_sink<std::string, size_t>>(thoughput_limited_streams, &std::cerr);
