...
int main(int argc, char **argv) {
...

  {
    ...
    auto sources = topology->create_partition_processors<kspp::kafka_source<void, std::string, kspp::text_codec>>(partition_list, "test_text", codec);
    std::regex rgx("\\s+");
  ...
  { 
    auto topology = text_builder.create_topic_topology();
    auto word_sources = topology->create_partition_processors<kspp::kafka_source<std::string, void, kspp::text_codec>>(partition_list, "test_words", codec);
