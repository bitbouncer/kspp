...
#ifdef KSPP_POSTGRES
#include <../libraries/kspp-pg/postgres_consumer.h>
#endif
...
namespace kspp {
  ...
  make_source(std::shared_ptr<cluster_config> config, std::shared_ptr<offset_storage> offset_store, std::string uri) {
    ...
    switch (parts.protocol) {
      ...
      case source_parts::KAFKA: {
        ...
        std::make_shared<kafka_source<K, V, avro_serdes, avro_serdes>>(config, parts.partition, parts.topic,
                                                                       config->avro_serdes(), config->avro_serdes());
      }
