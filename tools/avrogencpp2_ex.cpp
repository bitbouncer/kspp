...
//this is not a complete normalisation, it _should_ be but this should work for schema registry
...
public:
    CodeGen(std::ostream& os, const std::string& ns,
        ...
        const std::string& includePrefix, bool noUnion, bool enableTypeinfo) :
        ...
        enableTypeinfo_(enableTypeinfo), guardString_(guardString),
...
string CodeGen::generateRecordType(const NodePtr& n)
{
    ...
    if (enableTypeinfo_ && n->name().fullname() == root_name_)
    {
      os_ << "  //avro extension\n";
      ...
      os_ << "  static std::string                              full_name()        { return \"" << (inNamespace_ ? ns_ + "::" + n->name().fullname() : n->name().fullname()) << "\"; } \n";
    }
...
void CodeGen::generate(const ValidSchema& schema)
{
    if (enableTypeinfo_) {
      ...
      root_name_ = schema.root()->name().fullname();
    }
...
static const string NO_ENABLE_TYPEINFO_OUTPUT("enable-typeinfo-output");
...
int main(int argc, char** argv)
{
    ...
    desc.add_options()
        ...
        ("enable-typeinfo-output,x", "enable valid_schema() and fullname() in generated code")
    ...
    bool enableTypeinfo = vm.count(NO_ENABLE_TYPEINFO_OUTPUT) != 0;
    if (incPrefix == "-") {
        incPrefix.clear();
    ...
    try {
        ...
        if (! outf.empty()) {
            ...
            CodeGen(out, ns, inf, outf, g, incPrefix, noUnion, enableTypeinfo).generate(schema);
        } else {
            CodeGen(std::cout, ns, inf, outf, "", incPrefix, noUnion, enableTypeinfo).
                generate(schema);
        }
        return 0;
